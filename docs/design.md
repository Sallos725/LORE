...
- scope는 installation/user/character/chat/branch의 안정된 ID다. 자동 수집에는 host chaId/chat.id/message.chatId를 사용하고 count는 검증용 cursor로만 사용한다. Full은 기존 PocketRisu 로그인을 검증하고 저장 채팅의 안정 ID로 scope를 정한다. 별도 scope token은 없다. 기본 로그인 수집은 서버 /capture 전용이며 임의 revision 이벤트 전송을 허용하지 않는다.
...
- 컨텍스트 byte 상한과 UTF-8 기반 보수적 추정의 기억/전체 요청 예산은 별개다. 필수 문서 초과는 전체 주입을 중지한다. 기본 beforeRequest 주입은 공급자 변환 전 전체 요청과 응답 예약량을 추정한다. 선택적 최종 OpenAI 호환 body 검사도 제공한다. 다른 플러그인의 후속 변환과 provider의 내부 계산은 별도 한계다.
