...
export function readLoreDelta(character,cursor,{hashFactory,mode='delta',generating=false}={}) {
  ...
  const feed=(hash,message)=>{
    const fields=[message.memo??'',message.role??'',message.data??''];
  ...
  const prefix=hashFactory();for(let i=0;i<Math.min(cursor?.count??0,chat.message.length);i++)feed(prefix,chat.message[i]);
  ...
  const start=valid?(cursor?.count??0):0,hash=hashFactory(),messages=[],ids=new Set();let end=start,bytes=0;
  ...
  for(let i=start;i<Math.min(start+32,chat.message.length);i++){
    ...
    if(['user','char'].includes(message.role)&&message.data){
      if(typeof message.memo!=='string'||!message.memo||message.memo.length>160)throw Error('LORE: message needs a stable memo ID');
      ...
      if(bytes+size>60000)break;if(ids.has(message.memo))throw Error('LORE: duplicate message ID');ids.add(message.memo);
      messages.push({id:message.memo,text:message.data,role:message.role==='char'?'assistant':'user'});bytes+=size;
    }
