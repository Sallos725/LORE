...
def publish(api, dist, expected_tag, commit):
    ...
    if status == 404:
        release = checked(api.call('POST', '/releases', {
            ...
            'body': 'Lite runs in the browser; Full uses an HTTP(S) sidecar authenticated automatically with the existing PocketRisu login. No scope token or PocketRisu modifications/rebuilds are required. Each saved chat ID has its own wiki, sources, history and memory retrieval. Both editions support independent LLM providers, grounded extraction, hierarchical documents, aliases, links and manual corrections. Relevant wiki memory is retrieved from the current question and recent conversation, then injected through the common text-request hook with a conservative budget check. Lite includes notebook backup/restore; Full persists accepted jobs and extraction settings. Stock PocketRisu installation and actual Gemini request injection are tested. This is a public testing alpha: physical iPhone Safari/OOM, large narrative quality and all provider combinations remain unverified. See README, docs/automation.md, docs/public-readiness.md and SHA256SUMS.',
        }))
    else:
