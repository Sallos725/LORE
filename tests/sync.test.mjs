...
test('stable scope, missing IDs, oversized source and duplicate IDs fail safely',()=>{
 const c=character();delete c.chats[0].message[0].chatId;assert.throws(()=>readLoreDelta(c,null,{hashFactory}),/stable/);
 c.chats[0].message[0].chatId='m0';c.chats[0].message[0].data='한'.repeat(6000);assert.throws(()=>readLoreDelta(c,null,{hashFactory}),/16 KiB/);
});
test('actual PocketRisu Message chatId, comments and allBefore flags control collection',()=>{
 ...
 const first=readLoreDelta(character,null,{hashFactory});assert.deepEqual(first.messages.map(m=>m.id),['one']);
 ...
 assert.equal(readLoreDelta(character,first.cursor,{hashFactory,mode:'state'}).valid,false);
 const reset=readLoreDelta(character,first.cursor,{hashFactory});assert.equal(reset.reset,true);assert.deepEqual(reset.messages.map(m=>m.id),['now']);
 character.chats[0].message.at(-1).disabled=true;assert.equal(readLoreDelta(character,reset.cursor,{hashFactory,mode:'state'}).valid,false);
});
